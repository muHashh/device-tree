...
static int power_open(const hw_module_t __unused * module, const char *name, hw_device_t **device)
{
	...
	if (strcmp(name, POWER_HARDWARE_MODULE_ID) == 0) {
		struct sec_power_module *dev = (struct sec_power_module *)calloc(1,
			sizeof(struct sec_power_module));
		...
		if (dev) {
			...
			dev->base.init = sec_power_init;
			dev->base.powerHint = sec_power_hint;
